...
const CONFIG = {
  version: '4.0.0',
  spreadsheetId: '1K8SnQA8H2LVEnxyRTKYfTdLYRe2YHC4qQFqWUyOejQw', // Primary spreadsheet ID
  warrantyPeriod: 12,
...
function doGet(e) {
  ...
  if (e && e.parameter && (e.parameter.sheetId || e.parameter.spreadsheetId)) {
    ...
    if (sid) {
      try {
        ...
        console.warn('Failed to validate passed sheetId, using request-scoped ID only:', err.message);
        REQUEST_SPREADSHEET_ID = sid;
      }
    }
  }
  ...
  if (action === 'getVarmaData' || action === 'initVarmaSheets' || action === 'validateUser' || action === 'get_calls' || action === 'get_messages' || action === 'get_mobiles' || action === 'get_hc_numbers' || action === 'save_hc_number' || action === 'delete_hc_number' || action === 'verifyWarranty' || action === 'registerComplaint') {
    ...
    } else if (action === 'verifyWarranty') {
      const code = e && e.parameter ? String(e.parameter.productCode || '').trim() : '';
      result = verifyWarranty(code);
    } else if (action === 'registerComplaint') {
...
function getVarmaData() {
  try {
    ...
    // Smart check: only run syncIvrRegisteredUsers on dashboard load if sheet is empty or missing to avoid browser timeouts
    var registeredSheet = ss.getSheetByName('IVR_Registered_Users');
    var isRegSheetEmpty = !registeredSheet || registeredSheet.getLastRow() <= 1;
    if (isRegSheetEmpty) {
      try {
        syncIvrRegisteredUsers();
    ...
    if (registeredSheet) {
      var regValues = registeredSheet.getDataRange().getValues();
      for (var i = 1; i < regValues.length; i++) {
        var num = String(regValues[i][0]).trim();
...
/**
 * syncIvrRegisteredUsers
 * Gathers and consolidates all registered user phone numbers from users, parties, warranty,
 * technicians, dealers, and vendors sheets into a single consolidated sheet named 'IVR_Registered_Users'.
 */
function syncIvrRegisteredUsers() {
  try {
    var ss = getSpreadsheet();
    var sheetName = "IVR_Registered_Users";
    var sheet = ss.getSheetByName(sheetName);
    if (!sheet) {
      sheet = ss.insertSheet(sheetName);
    }
    ...
    function addRecord(phone, name, type, details, status, address) {
      if (!phone) return;
    ...
    try {
      ...
      warranty.forEach(function(w) {
        ...
        var address = String(w['Full Address'] || w.fulladdress || w.Address || w.address || '').trim();
    ...
    console.error('syncIvrRegisteredUsers error:', e);
...
/**
 * validateUser
 * Checks if phone number is registered in the consolidated IVR_Registered_Users sheet.
 */
...
/**
 * verifyWarranty
 * Searches the Warranty_Register sheet for the given product code (Warranty ID or Part Number).
 * Returns status: active, claimed, expired, or not_found.
 */
...
/**
 * registerIvrComplaint
 * Creates a complaint in the Complaints sheet for the given warrantyId and phone number.
 */
function registerIvrComplaint(warrantyId, phone, issue) {
  try {
    ...
    var newComplaint = {
      ...
      'Warranty ID': cleanWarrantyId,
      'Customer Name': customerName,
      'Mobile Number': cleanPhone,
      'Email': email,
      'Product Model': model,
      'Issue Type': issue,
      'Complaint Type': 'IVR Auto-Register',
      'Problem Description': 'Auto-registered via IVR system (Option 2 pressed by caller)',
    ...
    if (res.success) {
      ...
      return { success: false, error: res.message || 'Failed to save complaint' };
    }
  } catch (e) {
    console.error('registerIvrComplaint error:', e);
    return { success: false, error: e.message };
  }
}
